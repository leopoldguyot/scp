...
.checkMissingSamples <- function(sampleNotFound, carrierNotFound,
    ...
    if (length(sampleNotFound)) {
        msg <- paste0(
            msg, "No match found with 'samplePattern = \"",
            samplePattern, "\"' for the following set(s):\n",
            paste0(sampleNotFound, collapse = ", "), "\n"
        )
    }
    if (length(carrierNotFound)) {
        msg <- paste0(
            msg, "No match found with 'carrierPattern = \"",
            carrierPattern, "\"' for the following set(s):\n",
